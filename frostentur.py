...
def insert_weather_station_latlon():
    ...
    query = '''insert into fe_dw.weatherStationlatlon(sourceId, name, lat, lon) 
                        values (%s, %s, %s, %s)'''
...
def insert_busstop_route():
    ...
    for index, arg in stations.iterrows():
        ...
        if(index%100 == 0):
            query = '''insert into fe_star.route_busstop(route_id, bkey) 
                        values (%s, %s)'''
            cur.executemany(query, arg_list)
...
def insert_busstop_latlon():
    ...
    query1 = '''CREATE TABLE IF NOT EXISTS fe_dw.busstoplatlon(stop_id text, stop_name text, lat float, lon float);'''
    cur.execute(query1)

    query = '''insert into fe_dw.busstoplatlon(stop_id, stop_name, lat, lon) 
                        values (%s, %s, %s, %s)'''
    cur.executemany(query, arg_list)
...
def insert_routes():
    ...
    query = '''CREATE TABLE IF NOT EXISTS fe_dw.routes(route_id int,
    ...
    for index, arg in stations.iterrows():
        ...
        if(index%100 == 0):
            query = '''insert into fe_dw.routes(route_id, route_short_name, route_long_name, route_type) 
                        values (%s, %s, %s, %s)'''
            cur.executemany(query, arg_list)
            print(f'inserted: {index}')
            arg_list = []
...
def update_daily():
    ...
    query = '''insert into fe_star.facttable_1 (buss_stop_id, sourceId, temperatur, kdate, ktime)
                select bs.bkey, d.sourceid ,d.value ,d.datek, d.timek from fe_dw.DailyTemperatures d 
                join fe_star.closest_buss_weather_station cbws using(sourceId)
                join fe_star.buss_stopp bs on bs.buss_stopp_name = cbws.stop_name ;'''
    cur.execute(query)
    db.close(conn, cur)
...
def pythagoras_weather_buss():
    ...
    for _, row1 in buss_data.iterrows():
        ...
        for _, row2 in weather_data.iterrows():
            distance = sum(np.hypot((float(row1['stop_lat']), float(row1['stop_lon'])), (float(row2["lat"]), float(row2["lon"]))))
            print(distance)
            if(distance < min_distance[2]):
               min_distance =[row2["sid"], row2["sname"], row2["distance"]]
        closest_weather_station.append([row1["stop_id"], row1['stop_name']].append(min_distance))

    return closest_weather_station

print(pythagoras_weather_buss())